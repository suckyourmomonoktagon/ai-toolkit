...
| [`ci-check-pr-title.yml`](./ci-check-pr-title.yml) | Pull Request | Validates conventional PR titles; semantic validation is skipped for automated PRs (per `check-automated-pr`) and locally for `copilot/*` coding-agent branches, then re-run after Claude-generated metadata updates |                                                                                                 |
| [`claude-docs-check.yml`](./claude-docs-check.yml) | Pull Request | Validates PR documentation updates and forwards Claude auth to the reusable docs-check workflow, whose preflight skips safely when neither auth secret is configured                                                 |                                                                                                 |
