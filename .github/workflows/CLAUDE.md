...
**Missing auth behavior:** `_generate-pr-metadata.yml` now checks auth availability before generation. If neither secret is configured, it emits a notice and skips metadata generation steps instead of failing the workflow.
