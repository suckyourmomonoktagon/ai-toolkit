...
| `secrets.REVIEW_CLI_TOKEN`           | **Required.** A token with `packages:read` access. Both review-cli installation paths fail fast with a clear error when it is missing; if the secret is present but Bun still cannot install the package, the installer treats that as unavailable review-cli rather than a hard failure. |
