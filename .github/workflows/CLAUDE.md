...
- `claude-docs-check.yml` - Validates PR documentation is properly updated (CLAUDE.md, README, versions); skips when Claude authentication is unavailable
...
If both are provided, OAuth token takes precedence. If neither is configured, the documentation check skips successfully.
...
The reusable `generate-metadata` job also checks for actual secret presence and skips cleanly when neither credential is available, even if a caller leaves `auth_configured` at its default value.
