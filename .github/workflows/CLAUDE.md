...
If both are provided, OAuth token takes precedence. The top-level caller workflow runs a `check-authentication` preflight job and the reusable validator also checks for either credential; if neither is configured, the workflow reports a notice and skips validation successfully.
