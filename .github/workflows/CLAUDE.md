...
If both are provided, OAuth token takes precedence. A preflight job checks for either credential; if neither is configured, the workflow reports a notice and skips validation successfully. Callers should gate on that preflight output rather than referencing `secrets.*` in a job-level `if:`, because job-level conditions do not reliably support secret-based auth checks.
