...
| `secrets.REVIEW_CLI_TOKEN`           | **Required.** A token with `packages:read` access. The workflow now fails fast with an explicit error when it is missing; when it is present but cannot read the package, the installer still treats Bun's "403 but no installed package" path as unavailable review-cli rather than a hard failure. |
...
If both are provided, OAuth token takes precedence. The top-level caller forwards both secrets to `_claude-docs-check.yml`, whose `check-authentication` job reports a notice and skips validation successfully when neither credential is configured.
